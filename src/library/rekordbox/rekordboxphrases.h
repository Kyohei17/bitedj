...
inline Phrase phraseKind(const rekordbox_anlz_t::song_structure_entry_t& entry, int mood) {
    ...
    if (mood == 1) {
        ...
        switch (id) {
        case 1:
            phrase.kind = Phrase::Kind::Intro;
            phrase.label = "Intro";
            break;
        case 2:
            phrase.kind = Phrase::Kind::Up;
            phrase.label = "Up";
            break;
        case 3:
            phrase.kind = Phrase::Kind::Down;
            phrase.label = "Down";
            break;
        case 5:
            phrase.kind = Phrase::Kind::Chorus;
            phrase.label = "Chorus";
            break;
        case 6:
            phrase.kind = Phrase::Kind::Outro;
            phrase.label = "Outro";
            break;
        }
        ...
        if (id == 1) {
            ...
            phrase.kind = Phrase::Kind::Bridge;
            phrase.label = "Bridge";
        } else if (id == 9) {
            phrase.kind = Phrase::Kind::Chorus;
            phrase.label = "Chorus";
...
inline PhraseList decodePhrases(const rekordbox_anlz_t::song_structure_tag_t& tag,
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        if (phrase.endSeconds > phrase.startSeconds)

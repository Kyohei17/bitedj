...
inline QColor phraseColor(Phrase::Kind kind) {
    switch (kind) {
    case Phrase::Kind::Intro:
        return QColor(215, 30, 35);
    case Phrase::Kind::Verse:
        return QColor(48, 113, 73);
    case Phrase::Kind::Bridge:
        return QColor(121, 75, 145);
    case Phrase::Kind::Chorus:
        return QColor(35, 165, 20);
    case Phrase::Kind::Up:
        return QColor(145, 48, 220);
    case Phrase::Kind::Down:
        return QColor(170, 145, 70);
    case Phrase::Kind::Outro:
        return QColor(80, 135, 185);
    default:
        return QColor(80, 80, 80);
    }
}

inline void paintPhraseStrip(QPainter& painter,
        ...
        double last,
        double scale = 1,
        bool overviewRow = false) {
    if (phrases.isEmpty() || bounds.isEmpty() || !util_isfinite(first) ||
            !util_isfinite(last) || last <= first || !util_isfinite(scale) || scale <= 0)
        return;
    painter.save();
    ...
    for (const auto& phrase : phrases) {
        if (phrase.endSeconds <= first || phrase.startSeconds >= last)
            continue;
        ...
        if (!overviewRow && phrase.fillSeconds >= phrase.startSeconds &&
                phrase.fillSeconds < phrase.endSeconds) {
            painter.fillRect(QRectF(xAt(phrase.fillSeconds),
                                     y,
                                     end - xAt(phrase.fillSeconds),
                                     stripHeight),
        ...
        if (stripHeight >= 10 &&
                painter.fontMetrics().horizontalAdvance(phrase.label) +
                                6 * scale <
                        block.width()) {
